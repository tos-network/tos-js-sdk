import { Contract, createContract, ABI } from "./contract";
...
describe('Contract Class Tests', () => {
  ...
  describe('Dynamic Method Invocation', () => {
    test('should call addLiquidity with deposits', () => {
      ...
      expect(result.invoke_contract.deposits[token0]).toEqual({ amount: 1000000, private: false });
      expect(result.invoke_contract.deposits[token1]).toEqual({ amount: 2000000, private: false });
    });
    ...
    test('should call swap with correct parameters', () => {
      ...
      expect(result.invoke_contract.deposits[tokenIn]).toEqual({ amount: 1000000, private: false });
    });
  });
