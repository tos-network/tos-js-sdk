...
/**
 * Deposit configuration for contract calls
 */
export interface DepositConfig {
  amount: number | bigint;
  private?: boolean;  // TOS supports private deposits (default: false)
}

/**
 * Creates a deposits object for contract calls
 * @param deposits - Object mapping token hashes to amounts or deposit configs
 */
export function createDeposits(
  deposits: Record<string, number | bigint | DepositConfig>
): Record<string, { amount: number | bigint; private: boolean }> {
  const result: Record<string, { amount: number | bigint; private: boolean }> = {};

  for (const [tokenHash, depositValue] of Object.entries(deposits)) {
    // Validate hash format
    if (!/^[0-9a-fA-F]{64}$/.test(tokenHash)) {
      throw new Error(`Invalid token hash format: ${tokenHash}`);
    }

    // Handle both simple amount and full config
    if (typeof depositValue === 'object' && depositValue !== null && 'amount' in depositValue) {
      result[tokenHash] = {
        amount: depositValue.amount,
        private: depositValue.private ?? false
      };
    } else {
      result[tokenHash] = {
        amount: depositValue as number | bigint,
        private: false
      };
    }
  }
